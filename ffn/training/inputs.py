...
def weighted_load_patch_coordinates(
  ...
  Args:
    ...
    file_format: String indicating the format of the files in the queue.
                 Can be 'sstable' or 'tfrecord'. Defaults to 'sstable'.
...
def load_from_volume(
  ...
  def _load_data(coord, volname, patch_size, dtype, volinfo_map_string):
    volinfo_map = {}
    for pair in volinfo_map_string.split(','):
      label, path = pair.split(':')
      volinfo_map[label.strip()] = path.strip()

    volinfo_path = volinfo_map[volname]
    spec = {
        'driver': 'volumestore',
        'volinfo_path': volinfo_path,
    }

    # Open the TensorStore
    store = ts.open(spec, open=True).result()

    # Calculate start and stop coordinates for slicing
    start_coord = [c - (p // 2) for c, p in zip(coord, patch_size)]
    stop_coord = [c + (p // 2) + (p % 2) for c, p in zip(coord, patch_size)]

    # Slice and read the data
    data = store[start_coord[0]:stop_coord[0],
                 start_coord[1]:stop_coord[1],
                 start_coord[2]:stop_coord[2]].read().result()

    return data.transpose(2, 1, 0).astype(dtype.as_numpy_dtype())

  patch_size = list(patch_size)
  data = tf.py_function(
      _load_data,
      [coord, volname, patch_size, dtype, volinfo_map_string],
      Tout=dtype
  )
  data = tf.reshape(data, shape=[1] + patch_size[::-1] + [num_channels])
  return data
