...
class ObjectReviewStoreLocation(ObjectReview):
    ...
    def set_keybindings(self) -> None:
        ...
        self.viewer.actions.add(
            "merge0", lambda s: self.store_error_location(s, index=0, mode="merger")
        )
        self.viewer.actions.add(
            "merge1", lambda s: self.store_error_location(s, index=1, mode="merger")
        )
        self.viewer.actions.add(
            "split0", lambda s: self.store_error_location(s, index=0, mode="split")
        )
        self.viewer.actions.add(
            "split1", lambda s: self.store_error_location(s, index=1, mode="split")
        )
        self.viewer.actions.add(
            "delete_from_annotation", self.delete_location_from_annotation
        )
        self.viewer.actions.add(
            "delete_last_entry", lambda s: self.delete_last_location()
        )
        ...
        with self.viewer.config_state.txn() as s:
            ...
            s.input_event_bindings.viewer["control+keyv"] = "delete_from_annotation"
            s.input_event_bindings.viewer["control+keyz"] = "delete_last_entry"
    ...
    def store_error_location(
        ...
        if index == 1:
            if self.temp_coord_list[0] == self.temp_coord_list[1]:
                self.update_msg("You entered the same coordinate twice. Try again!")
                self.temp_coord_list = []
                return
    ...
    def mk_point_annotation(self, coordinate: list[int], annotation_id: str) -> None:
        ...
        if annotation_id.startswith("m"):
            ...
            color = "#05f2fa"
        annotation = neuroglancer.PointAnnotation(
