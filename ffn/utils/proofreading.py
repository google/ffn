...
class ObjectReviewStoreLocation(ObjectReview):
  ...
  def set_keybindings(self) -> None:
    ...
    self.viewer.actions.add(
        "merge0", lambda s: self.store_error_location(s, idx=0, mode="merger")
    )
    self.viewer.actions.add(
        "merge1", lambda s: self.store_error_location(s, idx=1, mode="merger")
    )
    self.viewer.actions.add(
        "split0", lambda s: self.store_error_location(s, idx=0, mode="split")
    )
    self.viewer.actions.add(
        "split1", lambda s: self.store_error_location(s, idx=1, mode="split")
    )
    self.viewer.actions.add(
  ...
  def store_error_location(
    ...
    if idx == 1 and not self.temp_coord_list:
